...
try {
    ...
    Write-Host "✅ Gateway LIVE: $($health.identity)" -ForegroundColor Green
    Write-Host "   Message: $($health.message)" -ForegroundColor Gray
} catch {
    Write-Host "❌ FAILED: Gateway not responding on port 8000" -ForegroundColor Red
...
try {
    ...
    if ($safety) {
        Write-Host "✅ Forge Status: $($safety.mode)" -ForegroundColor Green
        ...
        Write-Host "⚠️  Forge endpoint returned empty (skill may not be registered)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "⚠️  Forge Safety endpoint not available" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "✅ Knowledge Base Status:" -ForegroundColor Green
    ...
    Write-Host "⚠️  Could not retrieve KB status" -ForegroundColor Yellow
}
...
try {
    ...
    if ($isMock) {
        Write-Host "⚠️  WARNING: Response contains generic/mock patterns" -ForegroundColor Yellow
        ...
        Write-Host "✅ Response appears to be from LIVE Sovereign Stack" -ForegroundColor Green
    }
...
try {
    ...
    Write-Host "✅ Registered Skills:" -ForegroundColor Green
    foreach ($skill in $skills) {
        Write-Host "   • $($skill.name)" -ForegroundColor Gray
    }
} catch {
    Write-Host "⚠️  Could not retrieve skill registry" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "🔥 LIVE MODE SYNC COMPLETE 🔥" -ForegroundColor Cyan
Write-Host ""
