...
Write-Host "🚨 FORGE KILL SWITCH ACTIVATED" -ForegroundColor Red
...
if (Test-Path $envPath) {
    ...
    if ($envContent -match "PAGI_FORGE_SAFETY_ENABLED\s*=\s*false") {
        ...
        Write-Host "✓ PAGI_FORGE_SAFETY_ENABLED set to true in .env" -ForegroundColor Green
    }
    elseif ($envContent -match "PAGI_FORGE_SAFETY_ENABLED\s*=\s*true") {
        Write-Host "✓ PAGI_FORGE_SAFETY_ENABLED already set to true" -ForegroundColor Green
    }
    else {
        # Add the setting if it doesn't exist
        Add-Content -Path $envPath -Value "`nPAGI_FORGE_SAFETY_ENABLED=true"
        Write-Host "✓ PAGI_FORGE_SAFETY_ENABLED added to .env (set to true)" -ForegroundColor Green
    }
}
else {
...
if ($cargoProcesses) {
    $cargoProcesses | ForEach-Object {
        ...
        Write-Host "✓ Killed cargo process (PID: $($_.Id))" -ForegroundColor Green
    }
}
else {
    Write-Host "✓ No active cargo processes found" -ForegroundColor Green
}
...
if ($rustcProcesses) {
    $rustcProcesses | ForEach-Object {
        Stop-Process -Id $_.Id -Force
        Write-Host "✓ Killed rustc process (PID: $($_.Id))" -ForegroundColor Green
    }
}
...
try {
    ...
    Write-Host "✓ Emergency shutdown logged to KB-08" -ForegroundColor Green
}
catch {
...
Write-Host "✅ FORGE KILL SWITCH COMPLETE" -ForegroundColor Green
Write-Host ""
